...
*De novo* transcriptome assembly using the concatenated dataset resulted in 128,686 transcript $\gt 200$bp. The N50 of the transcriptome was 1,281 bp, and the median lenght of the assembled transcripts was `r median(meta_annotations$Length, na.rm=TRUE)` (MAD=`r mad(meta_annotations$Length, center=median(meta_annotations$Length), na.rm=TRUE, constant=1/quantile(meta_annotations$Length, probs=0.75, names=F))`). More details about the assembly can be found in Table 2 and the length distribution of the transcripts can be visualized in Figure 1.
...
Of the collection of transcripts, `r round(100*(sum(!is.na(meta_annotations$Protein))/(sum(is.na(meta_annotations$Protein))+sum(!is.na(meta_annotations$Protein)))),2)`% could be translated into proteins by Transdecoder. The majority of the translated transcripts were *Complete ORFs*, according to Transdecoder. The *Transdecoder ORF Types* distribution of these transcripts can be found in Table 3.
...
ggplot(meta_annotations_NoNAs, aes(ORF_Type)) + geom_bar(aes(fill=!is.na(Uniprot_match)), position="fill") + theme_bw() + labs(x="Transdecoder ORF Type", y="Frequency", fill="Annotation presence") + scale_x_discrete(labels=c("type:3prime_partial" = "3' Partial", "type:5prime_partial" = "5' Partial", "type:complete" = "Complete", "type:internal" = "Internal"))

```

**Figure 2.** Transcript length distribution of CBAS reference transcriptome. Maximum length allowed equal 10,000 base-pairs.
