...
kable(GenTable(under_functions_go_data, classic=under_resultsFisher_functions, topNodes=50))
```

Table 9
```{r Generate_Under_GO_Process_analysis_table, echo=FALSE}
kable(GenTable(under_process_go_data, classic=under_resultsFisher_process, topNodes=50))
```

Table 10
```{r Generate_Under_GO_Component_analysis_table, echo=FALSE}
kable(GenTable(under_component_go_data, classic=under_resultsFisher_component, topNodes=50))
```

#PFAM
```{r Pfam_over_enrichment_analysis, include=FALSE, results='hide'}
...
over_count_in_degs<-nrow(degs_pfam)
over_count_in_back<-nrow(back_pfam)
...
over_pfam_domain_enrichment_df<-data.frame(over_domain_col, over_domain_in_degs_col, over_domain_in_back_col, over_domain_pval_col)

```

```{r Pfam_under_enrichment_analysis, include=FALSE, results='hide'}
...
under_count_in_degs<-nrow(degs_pfam)
under_count_in_back<-nrow(back_pfam)
...
under_pfam_domain_enrichment_df<-data.frame(under_domain_col, under_domain_in_degs_col, under_domain_in_back_col, under_domain_pval_col)
