...
tpm_matrix_melted[,"Gen_set"]<-rep("All Genes", nrow(tpm_matrix_melted))
...
fpkm_matrix_melted[,"Gen_set"]<-rep("All Genes", nrow(fpkm_matrix_melted))
...
tpm_matrix_transdecoded_only_melted[,"Gen_set"]<-rep("Transdecoded Genes", nrow(tpm_matrix_transdecoded_only_melted))
...
fpkm_matrix_transdecoded_only_melted[,"Gen_set"]<-rep("Transdecoded Genes", nrow(fpkm_matrix_transdecoded_only_melted))
...
```
**Figure 7.** TPM and FPKM distribution in datasets including all trinity genes (All Genes) and only trinity genes that could be translated with trandescoder (Transdecoded Genes). In general, libraries should have similar TPM/FPKM distributions to be comparable.
...
```{r differential_expression_analysis_2, include=FALSE, results='hide'}
#get significant genes
degs_names<-rownames(subset(CBAS_DeSeq_Transdecoded_Only_NoOutliers_Results, padj<0.01 & abs(log2FoldChange) >= 2))
write.csv(degs_names, "~/Repos/CBAS/Results/DEGs/all_degs_p001_2LFC.csv")
...
write.csv(over_deg_names, "~/Repos/CBAS/Results/DEGs/over_degs_p001_2LFC.csv")
...
write.csv(under_deg_names, "~/Repos/CBAS/Results/DEGs/under_degs_p001_2LFC.csv")

```


```{r expression_heat_maps, echo=FALSE, fig.align='center'}

#merge under and over datasets with results object to sort by wald stat or p-value and produce a heat map.

#CBAS_DeSeq_Transdecoded_Only_NoOutliers_log2Counts_Over<-log2(counts(CBAS_DeSeq_Transdecoded_Only_NoOutliers, normalized=T)[rownames(CBAS_DeSeq_Transdecoded_Only_NoOutliers) %in% over_deg_names, ]+0.0001)

#CBAS_DeSeq_Transdecoded_Only_NoOutliers_Results_Over<-CBAS_DeSeq_Transdecoded_Only_NoOutliers_Results[rownames(CBAS_DeSeq_Transdecoded_Only_NoOutliers_Results) %in% over_deg_names, ]



#CBAS_DeSeq_Transdecoded_Only_NoOutliers_log2Counts_Over<-merge(CBAS_DeSeq_Transdecoded_Only_NoOutliers_log2Counts_Over,CBAS_DeSeq_Transdecoded_Only_NoOutliers_Results[rownames(CBAS_DeSeq_Transdecoded_Only_NoOutliers_Results) %in% over_deg_names, ], by="row.names", all=T)

#CBAS_DeSeq_Transdecoded_Only_NoOutliers_log2Counts_Over<-CBAS_DeSeq_Transdecoded_Only_NoOutliers_log2Counts_Over[order(CBAS_DeSeq_Transdecoded_Only_NoOutliers_log2Counts_Over$padj),]

#heatmap.2(CBAS_DeSeq_Transdecoded_Only_NoOutliers_log2Counts_Over[], scale="row", trace = "none", dendrogram = "column", col= colorRampPalette(rev(brewer.pal(9, "RdBu"))))


CBAS_DeSeq_Transdecoded_Only_NoOutliers_log2Counts_Under<-log2(counts(CBAS_DeSeq_Transdecoded_Only_NoOutliers, normalized=T)[rownames(CBAS_DeSeq_Transdecoded_Only_NoOutliers) %in% under_deg_names, ]+0.0001)


#merge(CBAS_DeSeq_Transdecoded_Only_NoOutliers_log2Counts_Under,CBAS_DeSeq_Transdecoded_Only_NoOutliers_Results[rownames(CBAS_DeSeq_Transdecoded_Only_NoOutliers_Results) %in% under_deg_names, ], by="row.names", all=T)
#heatmap.2(log2(counts(CBAS_DeSeq_Transdecoded_Only_NoOutliers, normalized=T)[rownames(CBAS_DeSeq_Transdecoded_Only_NoOutliers) %in% under_deg_names, ]+1), scale="row", trace = "none", dendrogram = "column", col= colorRampPalette(rev(brewer.pal(9, "RdBu"))))
