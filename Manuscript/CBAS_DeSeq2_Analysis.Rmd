...
degs_table<-subset(CBAS_DeSeq_Transdecoded_Only_NoOutliers_Results, padj<0.01 & abs(log2FoldChange) >= 2)
...
#change the log2FoldChange value above to 1 if the following results are to be generated
...
over_deg_table<-subset(CBAS_DeSeq_Transdecoded_Only_NoOutliers_Results, padj<0.01 & log2FoldChange >= 2)
...
under_deg_table<-subset(CBAS_DeSeq_Transdecoded_Only_NoOutliers_Results, padj<0.01 & log2FoldChange <= -2)
...
#uncomment the lfc cutoff you want to analyse
...
#generate table of results
kable(GenTable(over_functions_go_data, classic=over_resultsFisher_functions, topNodes=68))
```
...
#generate table of results
kable(GenTable(over_process_go_data, classic=over_resultsFisher_process, topNodes=124))
```
...
#generate table of results
kable(GenTable(over_component_go_data, classic=over_resultsFisher_component, topNodes=39))
```
...
kable(GenTable(under_functions_go_data, classic=under_resultsFisher_functions, topNodes=124))
```
...
kable(GenTable(under_process_go_data, classic=under_resultsFisher_process, topNodes=477))
```
...
kable(GenTable(under_component_go_data, classic=under_resultsFisher_component, topNodes=87))
```
...
#uncomment the lfc you want to use in the following analyses
